...
이 장의 로컬 실습은 `tutorials/CH06-Team-Collaboration-Conflict-and-Rollback-Lab/LAB.md`와 `tutorials/CH06-Team-Collaboration-Conflict-and-Rollback-Lab/variants/`를 기준으로 진행한다. 슬라이드 생성 시에는 tutorial 자산을 따로 업로드하지 않고, 아래 변형 문구와 역할별 흐름을 이 강의 노트 설명만으로 사용한다.

이번 장에서 참고하는 변형 문구:

- `tutorials/CH06-Team-Collaboration-Conflict-and-Rollback-Lab/variants/process-a-rewrite.md`
- `tutorials/CH06-Team-Collaboration-Conflict-and-Rollback-Lab/variants/process-b-rewrite.md`
- `tutorials/CH06-Team-Collaboration-Conflict-and-Rollback-Lab/variants/app-a.txt`
- `tutorials/CH06-Team-Collaboration-Conflict-and-Rollback-Lab/variants/app-b.txt`
...
- 강의 노트만으로 진행하는 순서:
  ...
  - 이 장은 본문 자체가 실행 절차다. `tutorials/CH06-Team-Collaboration-Conflict-and-Rollback-Lab/LAB.md`는 강사 진행표로만 본다
- 이 장에서 반드시 눈으로 확인할 것:
...
- `docs/process.md`의 1, 3, 4단계 번호 구조는 유지한다. CH07 smoke-check는 이 구조가 남아 있다고 가정한다.
...
`CH07 lecture note` 에서 merge 이후 파이프라인과 self-managed GitLab 운영 관점을 연결한다.
