...
- CI/CD와 MLOps의 차이를 구분하고, 이 교육이 어디까지 다루는지 설명할 수 있다.
- shared repository 모델과 role-based collaboration 모델을 이해할 수 있다.
- 내 환경의 인증 방식과 권한 수준을 확인할 수 있다.

## CI/CD와 MLOps는 어떻게 다른가

이 교육은 기본적으로 `Git -> GitLab 협업 -> MR/approval -> CI/CD -> rollback` 흐름을 중심으로 설계되어 있다. 다만 현업이 ML 서비스까지 포함한다면, 여기서 한 단계 더 확장한 MLOps 관점이 필요하다.

### CI/CD

- 코드 변경을 빌드, 테스트, 패키징, 배포 가능한 상태로 자동화하는 운영 흐름이다.
- 중심 질문은 “이 변경을 안전하게 반영할 수 있는가”다.
- 핵심 자산은 source code, test, build artifact, deploy pipeline이다.

### MLOps

- 코드뿐 아니라 모델, 데이터, 실험 결과, 평가 지표, 모델 아티팩트, 모델 배포를 함께 운영하는 체계다.
- 중심 질문은 “이 모델을 어떤 근거로 승격하고, 문제가 생기면 무엇을 되돌릴 것인가”다.
- 핵심 자산은 training pipeline, evaluation report, model artifact, model registry, serving configuration이다.

### 이 교육에서의 적용 범위

- CH01~CH06은 Git과 GitLab 협업의 공통 원리를 다룬다.
- CH07에서는 CI/CD를 self-managed runner와 quality gate 기준으로 읽는다.
- CH08에서는 capstone에 MLOps 확장 variant를 붙여 code rollback과 model rollback을 함께 생각한다.

즉, 일반적인 Git/GitLab 교육의 뼈대는 유지하되, 후반부에서 사내 환경과 ML 운영 현실을 반영해 확장한다.

## 튜토리얼 자산과 시작점

이 장의 로컬 실습은 `tutorials/CH01-Course-Foundation-and-Operating-Model/LAB.md`와 `tutorials/CH01-Course-Foundation-and-Operating-Model/seed_repo/`를 기준으로 진행한다. 슬라이드 생성 시에는 tutorial 파일을 따로 업로드하지 않고, 아래 자산의 역할을 이 강의 노트 설명만으로 해석한다.

이번 장에서 확인할 핵심 파일:

- `tutorials/CH01-Course-Foundation-and-Operating-Model/seed_repo/README.md`
- `tutorials/CH01-Course-Foundation-and-Operating-Model/seed_repo/src/permissions.js`
- `tutorials/CH01-Course-Foundation-and-Operating-Model/seed_repo/src/app.js`
- `tutorials/CH01-Course-Foundation-and-Operating-Model/seed_repo/docs/process.md`
- `tutorials/CH01-Course-Foundation-and-Operating-Model/seed_repo/tests/permissions.test.js`
...
- 강의 노트만으로 진행하는 순서:
  ...
  - 위 순서만 따라도 된다. `tutorials/CH01-Course-Foundation-and-Operating-Model/LAB.md`는 강사용 체크리스트로 본다
- 이 장에서 반드시 눈으로 확인할 것:
  - `origin` URL
  - 기본 브랜치 이름
  - `HEAD`가 무엇을 가리키는지
...
- CH07에서는 merge 이후 pipeline, self-managed runner, quality gate, CI/CD와 MLOps 확장 포인트를 읽는다.
- CH08에서는 issue부터 rollback까지 전 과정을 한 번에 연결하고, 필요 시 model promotion과 model rollback까지 확장해 본다.
...
## On-prem GitLab을 전제로 볼 때 달라지는 것

Git의 기본 개념과 대부분의 협업 원리는 GitLab.com이든 self-managed GitLab이든 크게 달라지지 않는다. 따라서 일반적인 Git/GitLab 수업 구조 자체는 여전히 유효하다. 다만 사내 on-prem 환경에서는 아래 차이가 뒤 장에서 실제 병목으로 나타난다.

- runner가 GitLab SaaS 기본 제공이 아니라 사내 인프라에 직접 등록되고 운영될 수 있다
- shell, docker, kubernetes executor 선택이 보안 정책과 네트워크 구조의 영향을 받는다
- 내부 인증서, proxy, 사설 registry, 폐쇄망 접근 제한 때문에 pipeline이 GitLab.com 예제처럼 동작하지 않을 수 있다
- GitLab 버전, runner 버전, 라이선스 tier 차이로 문서 기능이 일부 다를 수 있다
- variable, token, deploy 권한이 보안팀 또는 운영팀 승인 절차와 연결될 수 있다

따라서 이 교육은 전반부에서는 공통 원리를 가르치고, 후반부 CH07~CH08에서 self-managed 차이를 명시적으로 드러내는 방식이 가장 현실적이다.
...
```powershell
Copy-Item .\tutorials\CH01-Course-Foundation-and-Operating-Model\seed_repo\* . -Recurse
git status
...
### 사례 5. 갑자기 `bad object refs/desktop.ini` 또는 `broken ref refs/tags/desktop.ini`가 보인다

원인:

- Windows가 숨김 시스템 파일 `desktop.ini`를 생성했고, 드물게 `.git/refs` 내부까지 들어감
- 사용자는 문서 탐색만 했다고 느끼지만 Git 입장에서는 ref 파일 오염이다
...
- 저장소 루트의 `desktop.ini`는 `.gitignore`로 막을 수 있지만, `.git` 내부 오염은 ignore 규칙으로 해결되지 않는다
- 이 현상은 명령어 실수보다 운영체제 환경 이슈에 가깝다. 권한/브랜치 문제와 구분해서 진단해야 한다
...
`CH02 lecture note` 에서 실제로 파일을 수정하고, staging과 commit을 만들고, 원격에 반영하는 기본 루프를 완주한다.
