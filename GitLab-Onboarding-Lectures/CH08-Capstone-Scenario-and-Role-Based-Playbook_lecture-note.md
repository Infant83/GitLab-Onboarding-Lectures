...
- 필요 시 Git/GitLab capstone을 MLOps capstone으로 확장해 설명할 수 있게 만든다

## 튜토리얼 자산과 준비 파일

이 장의 로컬 실습은 `tutorials/CH08-Capstone-Scenario-and-Role-Based-Playbook/LAB.md`와 `tutorials/CH08-Capstone-Scenario-and-Role-Based-Playbook/assets/`를 기준으로 진행한다. 슬라이드 생성 시에는 tutorial 자산을 따로 업로드하지 않고, 아래 파일 이름과 capstone 흐름만 이 강의 노트 설명으로 해석한다.

이번 장에서 새로 추가하는 파일:

- `tutorials/CH08-Capstone-Scenario-and-Role-Based-Playbook/assets/issues/ISSUE-101-sample-action.md`
- `tutorials/CH08-Capstone-Scenario-and-Role-Based-Playbook/assets/src/sample-action.js`
- `tutorials/CH08-Capstone-Scenario-and-Role-Based-Playbook/assets/tests/sample-action.test.js`
- `tutorials/CH08-Capstone-Scenario-and-Role-Based-Playbook/assets/docs/release-decision-log.md`

확장 시나리오:

- 사내 OpenProject work package와 branch, MR, pipeline 상태를 연결하는 운영 variant를 함께 토론한다
...
- 강의 노트만으로 진행하는 순서:
  ...
  - `tutorials/CH08-Capstone-Scenario-and-Role-Based-Playbook/LAB.md`는 팀 진행자가 타임라인을 점검할 때만 본다
- 이 장에서 반드시 눈으로 확인할 것:
...
- OpenProject를 쓰는 팀이라면 work package reference를 branch와 MR에 남긴다
...
- Owner: code release 승인과 model promotion 승인을 구분해 판단한다
- Maintainer: evaluation 기준, model artifact 검증, registry 반영 여부를 확인한다
- Developer: training 결과, evaluation report, model version 근거를 MR에 남긴다
...
- OpenProject work package 또는 상위 기획 항목 reference
...
```bash
git switch -c feature/op-123-sample-action
```
...
- rollback 방법
- 관련 screenshot 또는 로그
- OpenProject work package reference 또는 연동 링크
...
## MLOps capstone variant

만약 이 저장소가 일반 애플리케이션이 아니라 ML 서비스의 일부라면, capstone은 아래처럼 확장될 수 있다.

- 기능 코드 변경
- 학습 또는 추론 관련 설정 변경
- evaluation report 생성
- model artifact 저장
- model registry 또는 내부 버전 표에 candidate 등록
- metric threshold 검토
- model promotion 승인
...
2. branch 생성
3. 코드 수정
4. training / evaluation pipeline 실행
5. metric review
6. model artifact 등록
7. MR / approval
8. pipeline + model gate 통과
9. merge
10. code rollback 또는 model rollback 판단

핵심 질문:

- 지금 되돌려야 하는 것은 코드인가 모델인가
- pipeline success만으로 충분한가, metric threshold도 통과했는가
- serving 중인 이전 모델로 빠르게 되돌릴 수 있는가

## OpenProject 연동 capstone variant

사내에서 OpenProject를 쓰는 경우 capstone은 아래까지 포함해 볼 수 있다.

- work package에서 요구사항 확인
- branch 이름에 work package 식별자 포함
- MR description에 work package reference 포함
- merge 후 OpenProject에서 linked MR, pipeline, discussion 상태 확인

핵심 질문:

- PM과 Owner가 GitLab에 직접 들어가지 않아도 현재 진행을 충분히 읽을 수 있는가
- work package와 MR이 연결되지 않으면 회고와 추적이 얼마나 어려워지는가
- webhook 장애가 나도 GitLab 승인과 배포 판단은 계속 가능한가
...
- ML 서비스라면 code release 승인과 model promotion 승인을 같은 단계로 취급하지 않는다
...
- self-managed runner, artifact, model registry 흐름을 함께 이해하고 infra 문제와 code 문제를 분리한다
...
- ML 관련 변경이라면 experiment 결과, metric, model version을 함께 제출한다
...
- 운영 runbook은 wiki에 둘지, repo docs에 둘지, 정적 안내는 Pages로 배포할지 기준이 있는가
...
- code rollback과 model rollback의 책임자가 같은가 다른가
- model registry 또는 이에 준하는 모델 버전 관리 절차가 존재하는가
- OpenProject work package, GitLab MR, pipeline 상태가 서로 연결되어 보이는가
