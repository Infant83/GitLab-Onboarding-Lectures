...
1. Developer A는 `process-a-rewrite.md`, `app-a.txt`를 참고해 기존 파일을 수정한다.
2. Developer B는 `process-b-rewrite.md`, `app-b.txt`를 참고해 같은 구간을 다르게 수정한다.
...
- CH07의 `scripts/smoke-check.js`는 `docs/process.md`에 1~4단계가 남아 있다고 가정하므로, conflict 해결 후에도 단계 번호를 지우지 않는다.
