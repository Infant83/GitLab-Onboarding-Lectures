...
이 장의 로컬 실습은 `tutorials/CH02-Local-Workflow-and-Core-Commands/LAB.md`와 `tutorials/CH02-Local-Workflow-and-Core-Commands/assets/`를 기준으로 진행한다. 슬라이드 생성 시에는 tutorial 파일을 따로 업로드하지 않고, 아래 파일 이름과 역할만 강의 노트에서 해석한다.

이번 장에서 새로 추가하는 파일:

- `tutorials/CH02-Local-Workflow-and-Core-Commands/assets/notes.txt`
- `tutorials/CH02-Local-Workflow-and-Core-Commands/assets/docs/tutorial-guide.md`
...
- 강의 노트만으로 진행하는 순서:
  ...
  - `tutorials/CH02-Local-Workflow-and-Core-Commands/LAB.md`는 강사용 빠른 체크용이다
- 이 장에서 반드시 눈으로 확인할 것:
  - `status`와 `diff` 출력 차이
...
```powershell
Copy-Item .\tutorials\CH02-Local-Workflow-and-Core-Commands\assets\notes.txt .\
Copy-Item .\tutorials\CH02-Local-Workflow-and-Core-Commands\assets\docs\tutorial-guide.md .\docs\
```
...
`CH03 lecture note` 에서 이력을 읽고, 실수와 회귀를 추적하고, 안전하게 되돌리는 방법을 다룬다.
